...
export function MyLandsPage() {
  ...
  const myLands = mockLands; // In real app, filter by current user
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">My Land Portfolio</h1>
          <p className="text-lg text-gray-600 mt-2">
            Manage your registered land properties and ownership records
          </p>
        </div>
        <div className="text-right">
          <p className="text-2xl font-bold text-gray-900">{myLands.length}</p>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {formatCurrency(myLands.reduce((sum, land) => sum + (land.price || 0), 0))}
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {myLands.filter(land => land.isForSale).length}
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {myLands.reduce((sum, land) => sum + land.size, 0).toFixed(1)}
                </p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {myLands.filter(land => land.status === 'registered').length}
                </p>
              </div>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between space-y-4 lg:space-y-0">
                <div className="flex-1 space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="text-xl font-semibold text-gray-900">{land.title}</h3>
                      <p className="text-sm text-gray-600 mt-1">Parcel ID: {land.parcelId}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2">
                        <div className={`w-2 h-2 rounded-full ${land.isForSale ? 'bg-emerald-500' : 'bg-gray-400'}`} />
                        <span className="font-medium">{land.isForSale ? 'For Sale' : 'Not Listed'}</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {myLands.length === 0 && (
        <Card>
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">No land properties found</h3>
            <p className="text-gray-600 mb-4">Start by registering your first land property</p>
        ...
        {selectedLand && (
          <div className="space-y-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <h4 className="font-semibold text-blue-900 mb-2">Property to Transfer</h4>
              <p className="text-blue-800">{selectedLand.title}</p>
              <p className="text-sm text-blue-700">Parcel ID: {selectedLand.parcelId}</p>
            </div>
            ...
            <div className="bg-amber-50 rounded-lg p-4">
              <p className="text-sm text-amber-800">
                <strong>Important:</strong> This action will transfer ownership of the property to the specified wallet address. 
